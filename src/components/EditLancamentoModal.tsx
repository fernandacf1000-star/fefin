...
: novoMesRef;
  ...
  const baseUpdates: Record<string, any> = {
    ...
    adriano: false, // principal is always adriano: false
  };
  if (isReceitaEdit) {
  ...
  // ── Handle Adriano split for existing parcelado ──
  if (wasParcelado && adrianoChanged && nowAdriano) {
    // Activating Adriano on parcelado: fetch affected parcelas, halve values, create mirrors
    let query = supabase
      .from("lancamentos")
      .select("*")
      .eq("parcelamento_id", lancamento.parcelamento_id!);
    if (editScope === "futuras") query = query.gte("data", lancamento.data);
    else if (editScope === "este") query = query.eq("id", lancamento.id);
    const { data: parcelas_db } = await query;
    const affected = (parcelas_db || []) as Lancamento[];

    const adrianoParcelamentoId = crypto.randomUUID();
    const mirrorRows: any[] = [];

    for (const p of affected) {
      // Update original: halve value
      await supabase
        .from("lancamentos")
        .update({
          ...baseUpdates,
          valor: numValor / 2,
          adriano: false,
          data: p.id === lancamento.id ? novaData : p.data,
          mes_referencia: p.id === lancamento.id ? mesRefFatura : p.mes_referencia,
        } as any)
        .eq("id", p.id);

      // Create mirror
      mirrorRows.push({
        descricao: baseUpdates.descricao,
        valor: numValor / 2,
        tipo: "despesa",
        categoria: lancamento.categoria || "extra",
        subcategoria: baseUpdates.subcategoria,
        categoria_macro: baseUpdates.categoria_macro,
        forma_pagamento: baseUpdates.forma_pagamento,
        cartao_id: baseUpdates.cartao_id,
        adriano: true,
    ...
    // Activating Adriano on recorrente
    let query = supabase
      .from("lancamentos")
      .select("*")
    ...
    const adrianoPaiId = crypto.randomUUID();
    const mirrorRows: any[] = [];
    ...
    for (const r of affected) {
      await supabase
        ...
        .update({
          ...
          valor: numValor / 2,
          adriano: false,
          data: r.id === lancamento.id ? novaData : r.data,
          mes_referencia: r.id === lancamento.id ? mesRefFatura : r.mes_referencia,
        } as any)
        .eq("id", r.id);
    ...
    // Activating Adriano on simple lancamento
    ...
    // Deactivating Adriano: restore value to double and delete mirrors
    // This case handles wasAdriano=true -> nowAdriano=false
    // But note: the principal lancamento already has adriano=false and half value
    // We need to double the value and delete mirrors
    // For simplicity, just update this lancamento and delete matching mirrors
    await updateLancamento.mutateAsync({
      id: lancamento.id,
      ...baseUpdates,
      valor: numValor * 2,
    ...
    // Normal edit of parcelado (no Adriano change)
    if (editScope === "este")
    ...
    // Normal edit of recorrente (no Adriano change)
    if (editScope === "este")
      await updateLancamento.mutateAsync({ id: lancamento.id, ...baseUpdates, data: format(data, "yyyy-MM-dd") });
    else if (editScope === "futuras")
...
<X size={17} className="text-muted-foreground" />
      ...
      {!isReceita && (
        <>
          ...
          {/* Toggle Luísa (só visível quando Adriano ativo) */}
          {isAdriano && (
            <button
