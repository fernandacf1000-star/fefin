...
// Helpers para cálculo 50/50 Adriano
// No banco, lançamentos com adriano=true já estão pela metade
function valorTotalCompartilhadoAdriano(l: { valor: number }) {
return Math.abs(Number(l.valor) || 0) * 2;
}

function valorParte50Adriano(l: { valor: number }) {
...
const totalPagoAdriano = despesasAdriano.reduce((s, l) => s + valorTotalCompartilhadoAdriano(l), 0);
...
const despesasPagasPorVoceAdriano = despesasAdriano.filter((l) =>
norm(l.pago_por) === “voce” || norm(l.pago_por) === “fernanda” || !norm(l.pago_por),
);

const despesasPagasPorEleAdriano = despesasAdriano.filter((l) => {
const p = norm(l.pago_por);
return p && p !== “voce” && p !== “fernanda”;
});

const totalPagoPorVoceAdriano = despesasPagasPorVoceAdriano.reduce(
(s, l) => s + valorTotalCompartilhadoAdriano(l),
0,
);

const totalPagoPorEleAdriano = despesasPagasPorEleAdriano.reduce(
(s, l) => s + valorTotalCompartilhadoAdriano(l),
0,
);

const adrianoDeve = despesasPagasPorVoceAdriano.reduce(
(s, l) => s + valorParte50Adriano(l),
0,
);

const voceDeve = despesasPagasPorEleAdriano.reduce(
(s, l) => s + valorParte50Adriano(l),
0,
);

const saldoAdriano = adrianoDeve - voceDeve - totalReembolsadoAdriano;
const meuCustoLiquidoAdriano = despesasAdriano.reduce(
(s, l) => s + valorParte50Adriano(l),
0,
);
...
const valorCategoria = aba === “adriano” && l.adriano
? valorTotalCompartilhadoAdriano(l)
: absValue(l.valor);
map.set(label, (map.get(label) || 0) + valorCategoria);
});
      ...
      {isLoading ? (
        ...
        listaAtual
          ...
          .map((l) => {
            ...
            const valor = aba === "adriano" && l.adriano
              ? valorTotalCompartilhadoAdriano(l)
              : absValue(l.valor);
            return (
              <div key={l.id} className="flex items-center gap-3 py-2.5 border-b border-slate-100 last:border-0">
                ...
                <div className="text-right shrink-0">
                  ...
                  {aba === "adriano" && !isLuisaLancamento(l) && (
                    <p className="text-[10px] text-slate-500">50% {fmt(aba === "adriano" && l.adriano ? valorParte50Adriano(l) : valor / 2)}</p>
                  )}
                </div>
              </div>
            );
